    ...
    for item in items:
        ...
        if isinstance(item, ToolCallItem):
            ...
            if isinstance(ret_val, ResponseFunctionToolCall):
                formatted_args = ', '.join(
                    f'{k}={repr(v)}' for k, v in json.loads(ret_val.arguments).items()
                )
        ...
        if event.type == 'raw_response_event':
            ...
            if event.item.type == 'tool_call_item':
                ...
                if isinstance(ret_val, ResponseFunctionToolCall):
                    formatted_args = ', '.join(
                        f'{k}={repr(v)}'
                        for k, v in json.loads(ret_val.arguments).items()
                    )
                    ret_val = f'{ret_val.name}({formatted_args})'
                print_and_write_to_file(
                    f'[{result.current_turn}]{"Tool Call":=^80}\n{ret_val}\n'
                )
            elif event.item.type == 'tool_call_output_item':
                print_and_write_to_file(
                    f'[{result.current_turn}]{"Tool Output":=^80}\n{(event.item.output)}\n'
                )
            elif event.item.type == 'message_output_item':
                print_and_write_to_file(
                    f'[{result.current_turn}]{"Agent":=^80}\n{ItemHelpers.text_message_output(event.item)}\n'
                )
            else:
                print_and_write_to_file(
                    f'[{result.current_turn}]{f"Unknown Item {event.item.type}":=^80}\n{event.item}\n'
                )
    return result.to_input_list()


def print_banner(
    model: str, temperature: float, max_turns: int, program_name: str
) -> None:
    print_and_write_to_file(
        banner(
            model=model,
